...
class WsReceiver(Receiver):
    def __init__(self, api: SessionApi, ping_timeout: Optional[int] = 5, sleep_time: Optional[int] = 3):
        super().__init__()
        self.api = api
                    ...
                    while True:
                        try:
                            ...
                            try:
                                ...
                                Logger.warning('Ping error - retrying connection in {} sec'.format(self.sleep_time))
                                await asyncio.sleep(self.sleep_time)
                                break
                        if res.get('code') == 10:
                            Logger.error('Websocket is not enabled!')
                            Logger.error('Retrying connection in {} sec'.format(self.sleep_time))
                            await asyncio.sleep(self.sleep_time)
                            break
                        await self._handle_event(res)
            except socket.gaierror:
                Logger.error('Socket error - retrying connection in {} sec'.format(self.sleep_time))
                await asyncio.sleep(self.sleep_time)
                break
